...
const jazzData: Record<string, Record<string, Show[]>> = require('../../data/JazzData.json');

const BlueNoteScreen = () => {
  ...
  const venueName = 'Blue Note NYC';

  const dates = Object.keys(jazzData);

  const [selectedDate, setSelectedDate] = useState<string>(initialSelectedDate);
  const [shows, setShows] = useState<Show[]>([]);

  useEffect(() => {
    const showsForDateAndVenue = jazzData[selectedDate]?.[venueName] || [];
    setShows(showsForDateAndVenue);
  }, [selectedDate]);  
